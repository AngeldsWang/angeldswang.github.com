...
<br />
简单说来就是$$\sigma$$-代数是定义在集合$$X$$上一个关于可数并集和补集运算封闭的集合系。其实这里称作集合代数更合适，可能是由于集合代数就算经过再怎么复杂运算后的结果还是逃脱不了集合系这一范围，所以这里就叫做集合系或者是集合代数是等价的。

###可测空间和可测函数
有了以上$$\sigma$$-代数的定义，就可以定义可测空间了。所谓的可测空间可以表示为$$(X,\cal{F})$$。直观上理解就是这个空间本身是由集合$$X$$和集合系$$\cal{F}$$共同决定的，而其可测性就是通过集合系$$\cal{F}$$具有$$\sigma$$-代数关于可数并集和补集运算封闭等性质来体现的。  
